...
let IncidenciasService = class IncidenciasService {
    ...
    estadoRepository;
    userRepository;
    constructor(incidenciaRepository, estadoRepository, userRepository) {
        this.incidenciaRepository = incidenciaRepository;
        this.estadoRepository = estadoRepository;
        this.userRepository = userRepository;
    }
    async create(createDto, userEmail) {
        ...
        if (!estadoPendiente) {
            throw new common_1.ForbiddenException('Estado pendiente no configurado');
        }
    ...
    async findAll(role, userEmail, query) {
        ...
        if (toDate)
            qb.andWhere('incidencia.fecha_creacion <= :toDate', { toDate });
    ...
    async getSoluciones(role, userEmail, query) {
        const page = query.page || 1;
        const limit = Math.min(query.limit || 10, 100);
        const skip = (page - 1) * limit;
        const qb = this.incidenciaRepository
    ...
    async update(idIncidencia, updateDto, userEmail, role) {
        ...
        if (updateDto.descripcion_solucion !== undefined && role !== 'admin') {
            throw new common_1.ForbiddenException('Solo el administrador puede registrar o editar la descripcion de solucion');
        }
    ...
    async updateEstado(idIncidencia, updateEstadoDto, userEmail, role) {
        if (role !== 'admin') {
            throw new common_1.ForbiddenException('Solo el administrador puede cambiar el estado y registrar la solucion');
        }
        const incidencia = await this.incidenciaRepository.findOne({
            where: { id_incidencia: idIncidencia },
            relations: ['usuario', 'estado'],
    ...
    async remove(idIncidencia, role) {
        if (role !== 'admin') {
            throw new common_1.ForbiddenException('Solo el administrador puede eliminar incidencias');
        }
    ...
    async getReportes(role, query, reportByEmail) {
        if (role !== 'admin') {
            throw new common_1.ForbiddenException('Solo el administrador puede ver reportes');
        }
        return this.buildReportData(query, reportByEmail);
    }
    async getReportesPdf(role, query, reportByEmail) {
        if (role !== 'admin') {
            throw new common_1.ForbiddenException('Solo el administrador puede ver reportes');
        }
        ...
        const xPositions = columns.reduce((acc, col, idx) => {
            if (idx === 0) {
                acc.push(left);
            }
            else {
        ...
        const drawColumnSeparators = (y, height) => {
            doc.save();
        ...
        const getRowHeight = (item) => {
            ...
            const heights = columns.map((col) => doc.heightOfString((item[col.key] || '-').toString(), {
                width: col.width - cellPaddingX * 2,
                lineGap: 0,
            }));
            return Math.max(minRowHeight, Math.ceil(Math.max(...heights) + cellPaddingY * 2));
        };
        const drawTableRow = (item, y, index, rowHeight) => {
        ...
        report.detalles.forEach((item, index) => {
            ...
            drawTableRow(item, currentY, index, rowHeight);
            currentY += rowHeight;
        });
        doc.y = currentY;
        doc.end();
    ...
    async buildReportData(query, reportByEmail) {
        const qb = this.incidenciaRepository
            .createQueryBuilder('incidencia')
            .leftJoinAndSelect('incidencia.estado', 'estado')
            .leftJoinAndSelect('incidencia.usuario', 'usuario');
    ...
    ensureCanEdit(incidencia, userEmail, role) {
    ...
    getDateRange(from, to) {
        ...
        if (from) {
            ...
            if (Number.isNaN(fromDate.getTime())) {
                throw new common_1.BadRequestException('Parámetro from inválido (usa YYYY-MM-DD)');
            }
        }
        if (to) {
            toDate = new Date(to);
            if (Number.isNaN(toDate.getTime())) {
                throw new common_1.BadRequestException('Parámetro to inválido (usa YYYY-MM-DD)');
            }
